...
$spreadsheet->getProperties()->SetCreator('SysVakilact')->SetTitle('Reporte Vendedores')->setCategory('Gestión')->setCompany('Lácteos Vakilact');

$spreadsheet->setActiveSheetIndex(0);
$hojaActiva = $spreadsheet->getActiveSheet();
$hojaActiva->setTitle("Vendedores");
...
if(empty($nombreSede = $_POST['slctSedes']) or empty($nombrePuesto = $_POST['slctPuesto']) or empty($fReg = $_POST['txtFreg']) or empty($fSal = $_POST['txtFsal'])){
    if(!empty($nombreSede = $_POST['slctSedes']) and !empty($nombrePuesto = $_POST['slctPuesto']) and !empty($fReg = $_POST['txtFreg'])){
        $consulta = "SELECT * FROM vendedor WHERE ID_SEDE = '$nombreSede' AND PUESTO = '$nombrePuesto' AND FECHA_REGISTRO >= '$fReg'";
    }elseif(!empty($nombreSede = $_POST['slctSedes']) and !empty($nombrePuesto = $_POST['slctPuesto']) and !empty($fSal = $_POST['txtFsal'])){
        $consulta = "SELECT * FROM vendedor WHERE ID_SEDE = '$nombreSede' AND PUESTO = '$nombrePuesto' AND FECHA_SALIDA >= '$fSal'";    
    }elseif(!empty($nombreSede = $_POST['slctSedes']) and !empty($fReg = $_POST['txtFreg']) and !empty($fSal = $_POST['txtFsal'])){
        $consulta = "SELECT * FROM vendedor WHERE ID_SEDE = '$nombreSede' AND FECHA_REGISTRO >= '$fReg' AND FECHA_SALIDA >= '$fSal'";  
    }elseif(!empty($nombrePuesto = $_POST['slctPuesto']) and !empty($fReg = $_POST['txtFreg']) and !empty($fSal = $_POST['txtFsal'])){
        $consulta = "SELECT * FROM vendedor WHERE PUESTO = '$nombrePuesto' AND FECHA_REGISTRO >= '$fReg' AND FECHA_SALIDA >= '$fSal'";  
    }elseif(!empty($nombreSede = $_POST['slctSedes']) and !empty($nombrePuesto = $_POST['slctPuesto'])){
        $consulta = "SELECT * FROM vendedor WHERE ID_SEDE = '$nombreSede' AND PUESTO = '$nombrePuesto'";  
    }elseif(!empty($nombreSede = $_POST['slctSedes']) and !empty($fReg = $_POST['txtFreg'])){
        $consulta = "SELECT * FROM vendedor WHERE ID_SEDE = '$nombreSede' AND FECHA_REGISTRO >= '$fReg'";  
    }elseif(!empty($nombreSede = $_POST['slctSedes']) and !empty($fSal = $_POST['txtFsal'])){
        $consulta = "SELECT * FROM vendedor WHERE ID_SEDE = '$nombreSede' AND FECHA_SALIDA >= '$fSal'";  
    }elseif(!empty($nombrePuesto = $_POST['slctPuesto']) and !empty($fReg = $_POST['txtFreg'])){
        $consulta = "SELECT * FROM vendedor WHERE PUESTO = '$nombrePuesto' AND FECHA_REGISTRO >= '$fReg'";
    }elseif(!empty($nombrePuesto = $_POST['slctPuesto']) and !empty($fSal = $_POST['txtFsal'])){
        $consulta = "SELECT * FROM vendedor WHERE PUESTO = '$nombrePuesto' AND FECHA_SALIDA >= '$fSal'";  
    }elseif(!empty($fReg = $_POST['txtFreg']) and !empty($fSal = $_POST['txtFsal'])){
        $consulta = "SELECT * FROM vendedor WHERE FECHA_REGISTRO >= '$fReg' AND FECHA_SALIDA >= '$fSal'";  
    }elseif(!empty($nombreSede = $_POST['slctSedes'])){
        $consulta = "SELECT * FROM vendedor WHERE ID_SEDE = '$nombreSede'";
    }elseif(!empty($nombrePuesto = $_POST['slctPuesto'])){
        $consulta = "SELECT * FROM vendedor WHERE PUESTO = '$nombrePuesto'";
    }elseif(!empty($fReg = $_POST['txtFreg'])){
        $consulta = "SELECT * FROM vendedor WHERE FECHA_REGISTRO >= '$fReg'"; 
    }elseif(!empty($fSal = $_POST['txtFsal'])){
        $consulta = "SELECT * FROM vendedor WHERE FECHA_SALIDA >= '$fSal'";  
    }else{
    ...
    $consulta = "SELECT * FROM vendedor WHERE ID_SEDE = '$nombreSede' AND PUESTO = '$nombrePuesto' AND FECHA_REGISTRO >= '$fReg' AND FECHA_SALIDA >= '$fSal'";
}else{
...
$hojaActiva->setCellValue('D1', 'Reporte Vendedores');
...
header('Content-Disposition: attachment;filename="Reporte Vendedores.xlsx"');
...
header('Content-Disposition: attachment;filename="Reporte vendedor.xls"');
header('Cache-Control: max-age=0');
...
$writer->Save('Reporte vendedor.xlsx');
*/
?>